...
class Inference:
    ...
    def load_model(self, checkpoint):
        ...
        # Convert model to appropriate device and precision for faster inference
        ...
        self.model.pe_embedding = self.model.pe_embedding.to(device=device, dtype=precision)
        self.model.binary_decoder.requires_grad = False
        self.model.eval()
        log.info(f"SE model device: {next(self.model.parameters()).device}")
    ...
    def encode(self, dataloader, rda=None):
        ...
        with torch.no_grad():
            # Use the actual model device, not just torch.cuda.is_available()
            device = next(self.model.parameters()).device
            device_type = "cuda" if device.type == "cuda" else "cpu"
            precision = get_precision_config(device_type=device_type)

            log.info(f"Encoding on device: {device}")

            with torch.autocast(device_type=device_type, dtype=precision):
                for i, batch in enumerate(dataloader):
                    # Move every tensor in the dataloader batch to the model device
